...
/*   Updated: 2023/10/27 14:31:10 by jhendrik      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	delete_pair(t_htable *env, char *key)
{
	...
	to_del = find_env_valuenode(env, key); // seems okay, just need to change unset to check return value
	index = give_hash_index(key, env);
...
int	add_pair(t_htable *env, char *key, char *value)
{
	...
	if (env->array == NULL)
		return (-1);
	new = new_hash_node(key, value); // seems okay, just need to change export to check value
	index = give_hash_index(key, env);
	if (new == NULL)
		return (-2);
	if (index < 0)
