...
/*   Updated: 2023/10/25 16:39:16 by jhendrik      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
#include "minishell.h"
...
t_hnode	**init_hash_array(t_htable *env_table, char **envp)
{
	...
	if (env_table == NULL || envp == NULL)
		return (NULL);
	head = hasharray_calloc((env_table->size) + 1, sizeof(t_hnode *)); //good
	if (head == NULL)
		return (NULL);
	check = init_keyvalue(&head, env_table, envp); //good
	if (check < 0)
		return (NULL);
	return (head);
}
...
t_htable	*init_env(char **envp)
{
	...
	if (env_table == NULL)
		return (NULL);
	env_table->size = 19;
	env_table->array = init_hash_array(env_table, envp); // good
