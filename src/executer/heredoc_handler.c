...
/*   Updated: 2023/10/27 15:37:34 by jhendrik      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static char	*st_give_filename(char *s_nb1, int j)
{
	...
	if (s_nb1 == NULL)
		return (NULL);
	s_nb2 = ft_itoa_base(j, 16, "0123456789ABCDEF"); // good now, s_nb1 was freed twice before, now only free it where it is created
	if (s_nb2 == NULL)
		return (NULL);
